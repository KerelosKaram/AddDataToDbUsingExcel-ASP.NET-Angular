...
@Component({
  selector: 'app-igp-item-elamir',
  templateUrl: './igp-item-elamir.component.html',
  styleUrls: ['./igp-item-elamir.component.scss']
})
export class IgpItemElamirComponent {
  ...
  downloadFile(): void {
    ...
    this.http
      .get(`${environment.apiUrl}/template/download?templateName=IGPItemElamir`, {
        responseType: 'blob',
        headers,
      })
      .subscribe(
  ...
  addDataToTable(): void {
    ...
    const url = `${environment.apiUrl}/Excel/Sql2017/insertdata?fileName=${this.fileName}&tableName=IGPItemElamir`;
  ...
  deleteTableFromDatabase(): void {
    ...
    const url = `${environment.apiUrl}/Excel/Sql2017/deletedata?tableName=IGPItemElamir`;
    console.log(`Sending request to: ${url}`);
