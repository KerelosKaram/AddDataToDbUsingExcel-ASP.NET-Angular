...
@Component({
  selector: 'app-igp-salesman-target',
  templateUrl: './igp-salesman-target.component.html',
  styleUrls: ['./igp-salesman-target.component.scss']
})
export class IgpSalesmanTargetComponent {
  ...
  downloadFile(): void {
    ...
    this.http
      .get(`${environment.apiUrl}/template/download?templateName=IGPSalesmanTarget`, {
        responseType: 'blob',
        headers,
      })
      .subscribe(
        (response) => {
          ...
          a.href = url;
          a.download = 'IGPSalesmanTarget_API.xlsx'; // Set the default name for the downloaded file
          a.click();
  ...
  addDataToTable(): void {
    ...
    const url = `${environment.apiUrl}/Excel/Sql2017/insertdata?fileName=${this.fileName}&tableName=IGPSalesmanTarget`;
  ...
  deleteTableFromDatabase(): void {
    ...
    const url = `${environment.apiUrl}/Excel/Sql2017/deletedata?tableName=IGPSalesmanTarget`;
    console.log(`Sending request to: ${url}`);
