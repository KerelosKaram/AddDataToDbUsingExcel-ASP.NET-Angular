...
@Component({
  selector: 'app-qs-customer-brand-target',
  templateUrl: './qs-customer-brand-target.component.html',
  styleUrls: ['./qs-customer-brand-target.component.scss']
})
export class QsCustomerBrandTargetComponent {
  ...
  downloadFile(): void {
    ...
    this.http
      .get(`${environment.apiUrl}/template/download?templateName=QSCustomerBrandTarget`, {
        responseType: 'blob',
        headers,
      })
      .subscribe(
        (response) => {
          ...
          a.href = url;
          a.download = 'QSCustomerBrandTarget_API.xlsx'; // Set the default name for the downloaded file
          a.click();
  ...
  addDataToTable(): void {
    ...
    const url = `${environment.apiUrl}/Excel/Sql2017/insertdata?fileName=${this.fileName}&tableName=QSCustomerBrandTarget`;
